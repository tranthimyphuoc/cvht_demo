/* Demo — BT→CVHT · LT→CVHT · LT NN→CVHT · CVHT→QLĐT */
const DEMO_ACCOUNTS = [
...
const DemoKit = {
  seedFullScenario(actor) {
    ...
    Store.update((d) => {
      ...
      d.reports.unshift({
        id: btId, demoTag: true, classId: 'c1', subject: 'Quản trị Kinh doanh', reporterId: 'u_bt',
      ...
      d.reports.unshift({
        id: ltId, demoTag: true, classId: 'c1', subject: 'Quản trị Kinh doanh', reporterId: 'u_lt',
        reportKind: 'LOP_TRUONG', reportType: 'TUAN',
        weekStart: range.start.toISOString(), weekEnd: range.end.toISOString(),
        status: 'SENT_TO_CVHT', formData: lt.formData, totalScore: lt.totalScore,
        isLate: false,
        summaryNote: '3 SV nguy cơ đang theo dõi. Đề nghị CVHT hỗ trợ case mất liên lạc.',
        createdAt: now, submittedAt: now, recipientRole: 'CVHT',
      });
      ...
      d.visits.unshift({
        id: Store.uid('vis'), demoTag: true, classId: 'c1', cvhtId: 'u_nq',
        visitDate: now.slice(0, 10),
        observation: 'Lớp ổn định, sĩ số tốt. Nhắc LT đôn đốc 2 SV nghỉ học.',
        createdAt: now,
      });

      d.reports.unshift({
        id: Store.uid('rp'), demoTag: true, classId: 'c1', subject: 'Quản trị Kinh doanh', reporterId: 'u_nq',
        ...
        formData: {
          visitDone: true,
          classMood: 'Ổn định',
          riskSummary: '3 SV nguy cơ CN + 1 SV NN — đã liên hệ phần lớn',
        ...
        attachments: [
          ...
          },
          {
            ...
            size: 86,
            kind: 'file',
            dataUrl: 'data:text/plain;base64,' + btoa('Tom tat tuan — lop on dinh, 3 SV nguy co dang theo doi.'),
          },
        ],
      ...
      [
        { userId: 'u_nq', title: '[Demo] BC Bí thư mới', body: 'HN-K25-QTKD1 — Lê Thu Hà đã gửi báo cáo hoạt động.' },
        { userId: 'u_nq', title: '[Demo] BC Lớp trưởng CN', body: 'HN-K25-QTKD1 — Trần Minh Đức đã gửi báo cáo tuần.' },
        ...
        { userId: 'u_admin', title: '[Demo] BC tổng hợp CVHT', body: 'Nguyễn Thị Như Quỳnh đã gửi báo cáo tổng hợp.' },
      ].forEach((n) => {
